  ...
  const saveDetails = () => {
    ...
    try {
      if (!title || !calories || !imageAsset || !price || !category) {
        ...
        const data = {
          id: `$Date.now()`, title: title, imageURL: imageAsset, category: category, calories: calories, qty: 1, price: price,
        }
        saveItem(data)
        setIsLoading(false);
        setFields(true);
      ...
      <div className='w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4'>
        {fields && (
          <motion.p
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`w-full p-2 rounded-lg text-center text font-semibold ${alertStatus === "danger"
